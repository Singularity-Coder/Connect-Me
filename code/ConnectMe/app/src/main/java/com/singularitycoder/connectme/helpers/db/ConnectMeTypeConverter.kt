...
abstract class ConnectMeTypeConverter<T> {
    ...
    @TypeConverter
    fun listToString(value: List<T>): String = ConnectMeUtils.gson.toJson(value, type)

    @TypeConverter
    fun stringToList(value: String): List<T> = ConnectMeUtils.gson.fromJson(value, type)
}
